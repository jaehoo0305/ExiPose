...
namespace BLINK.Tools
{
    public class MaterialTilingOffset : EditorWindow
    {
        ...
        private void Init()
        {
            ...
            {
                Debug.LogWarning("GameObject List is empty.");
                return;
            }

            Renderer renderer = gameObjectList[0].GetComponentInChildren<Renderer>(); //  여기 수정
            if (renderer != null)
            {
                ...
                {
                    Renderer r = weapon.GetComponentInChildren<Renderer>(); // 여기도 수정
                    if (r == null) continue;
                    r.sharedMaterial = newMaterial;
                }

                UpdateMaterial();
                Debug.Log(" Material initialized successfully!");
            }
            else
            {
                Debug.LogError(" No Renderer found in the selected GameObject or its children.");
            }
        }
